...
class Weather_manager:
    async def make_request(self, query, location: str = None, retries: int =3, ttl: int = None):
        ...
        if location is None:
            ...
            if not location:
                    error_logger.error("Unable to fetch no location to query weather data from.")
                    return send_message(False, message='no location')

        data = cache.get_cache(location)
        if data:
            if query in data:
        ...
        cache.set_cache(location, data)
        return send_message(True, data=data)
        

    def get_current_weather_table(self, data, temp = "c", speed= 'km'):
    ...
    def get_weather_forecast_table(self, data, temp='f', speed='km'):
        ...
        # --------- Km filter -----------------
        if speed in ('miles', 'm'):
            table.add_row(f"Max wind speed: {day.get('maxwind_kph', {})}kmph")
